            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="px-4 py-2 border-b border-gray-100 flex items-center justify-between">
                        ...
                        @if(auth()->user()->unreadNotifications->count() > 0)
                            <a href="{{ route('typing.notifications.readAll') }}"
                                class="text-xs text-primary-600 hover:text-primary-700">อ่านทั้งหมด</a>
                        @endif
                    </div>
                    <div class="max-h-64 overflow-y-auto">
                        @forelse(auth()->user()->unreadNotifications as $notification)
                            <a href="{{ route('typing.notifications.read', $notification->id) }}"
                                class="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0 relative group">
                                <div
                                    class="w-8 h-8 rounded-full {{ $notification->data['color'] ?? 'bg-gray-100 text-gray-500' }} flex items-center justify-center flex-shrink-0">
                                    <i class="{{ $notification->data['icon'] ?? 'fas fa-bell' }} text-sm"></i>
                                </div>
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-800">
                                        {{ $notification->data['title'] ?? 'การแจ้งเตือน' }}</p>
