    ...
    <div class="space-y-6">
        ...
        <div class="card p-6">
            <form action="{{ route('typing.admin.assignments.update', $assignment->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="col-span-2 md:col-span-1">
                        ...
                        <input type="text" name="chapter" id="chapter" class="form-input w-full"
                            value="{{ old('chapter', $assignment->chapter) }}">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="form-select w-full">
                            <option value="internal" {{ $assignment->type == 'internal' ? 'selected' : '' }}>หนังสือภายใน
                                (Internal)</option>
                            <option value="external" {{ $assignment->type == 'external' ? 'selected' : '' }}>หนังสือภายนอก
                                (External)</option>
                            <option value="command" {{ $assignment->type == 'command' ? 'selected' : '' }}>คำสั่ง
                                (Command)</option>
                            <option value="memo" {{ $assignment->type == 'memo' ? 'selected' : '' }}>บันทึกข้อความ (Memo)
                            </option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="max_score" id="max_score"
                            value="{{ old('max_score', $assignment->max_score) }}" class="form-input w-full">
                    </div>
                    ...
                    @if($assignment->submission_type === 'typing')
                        <div>
                            ...
                            <input type="number" name="time_limit" id="time_limit" min="1" max="60"
                                value="{{ old('time_limit', $assignment->time_limit ?? 5) }}" class="form-input w-full">
                ...
                <!-- Content -->
                <div class="mb-6">
                    <label for="content" class="block text-sm font-medium text-gray-700 mb-1">เนื้อหาที่ต้องพิมพ์
                        (สำหรับโหมดพิมพ์ในระบบ)</label>
                    <textarea name="content" id="content" rows="8"
                        class="form-textarea w-full font-mono text-base">{{ old('content', $assignment->content) }}</textarea>
                </div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="font-medium text-gray-800">
                                        {{ $assignment->master_file_name ?? 'ไฟล์ต้นฉบับ' }}</p>
                    ...
                    <div class="mt-4 pt-4 border-t border-blue-200">
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">ตรวจฟอนต์ (TH SarabunPSK 16pt), ระยะขอบ, ขนาดกระดาษ,
                                    หัวเรื่อง</p>
                            </div>
                        </label>
                    </div>
