<x-typing-app :role="'admin'" :title="'จัดการงาน - ระบบวิชาพิมพ์หนังสือราชการ 1'">
    ...
    @if(session('success'))
        <div
            class="mb-6 p-4 bg-green-100 border-l-4 border-green-500 text-green-700 rounded shadow-sm flex items-center justify-between">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        @forelse($assignments as $assignment)
            <div
                class="card p-0 overflow-hidden border-t-4 {{ $assignment->is_active ? 'border-secondary-500' : 'border-gray-400' }} hover:shadow-lg transition-shadow">
                <div class="p-5">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open" @click.away="open = false"
                                ...
                                <form action="{{ route('typing.admin.assignments.destroy', $assignment->id) }}"
                                    method="POST" class="block w-full text-left" onsubmit="return confirm('ยืนยันการลบ?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    <div class="mb-1">
                        <span
                            class="text-xs font-semibold text-primary-700 bg-primary-100 px-2 py-0.5 rounded-full inline-block">
                            {{ $assignment->chapter ?: 'บทเรียนทั่วไป' }}
                        </span>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-800 mb-2 truncate" title="{{ $assignment->title }}">
                        {{ $assignment->title }}</h3>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span
                                class="font-medium {{ $assignment->due_date && $assignment->due_date->isPast() ? 'text-red-600' : 'text-gray-800' }}">
                                {{ $assignment->due_date ? $assignment->due_date->format('d/m/Y') : '-' }}
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-500">จำนวนที่ส่ง</span>
                            <span class="font-medium text-gray-800">{{ $assignment->submissions->count() }}
                                Submissions</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        @endphp
                        <div class="progress">
                            <div class="progress-bar bg-gradient-to-r {{ $assignment->is_active ? 'from-secondary-500 to-secondary-400' : 'from-primary-500 to-primary-400' }}"
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100">
                        <a href="{{ route('typing.admin.submissions', ['assignment_id' => $assignment->id]) }}"
                            class="w-full btn-outline justify-center text-sm py-2">
        ...
        <a href="{{ route('typing.admin.assignments.create') }}"
            class="card p-0 overflow-hidden border-2 border-dashed border-gray-300 hover:border-primary-400 transition-colors cursor-pointer group">
            <div class="p-5 h-full flex flex-col items-center justify-center text-center min-h-[280px]">
                <div
                    class="w-16 h-16 rounded-full bg-gray-100 group-hover:bg-primary-100 flex items-center justify-center mb-4 transition-colors">
